...
def has_well_defined_breakpoint(left_seq: str, right_seq: str) -> bool:
    ...
    for n in range(2,6):
        repetitive_sequence = full_seq[:n].seq()*max(2,1+int(11/n))
...
def clean_clipped_seq(seq: QualitySeq) -> QualitySeq:
    # remove poly G at tail of read
    if len(seq) < 2:
        return QualitySeq('',[])
    i = len(seq)
    while i > 1 and seq.seq(i-1) == 'G':
        i -= 1
    if i < len(seq)-4:
        seq = seq[:i]
    ...
    j = 0
    while i < len(seq):
        if seq.seq(i) == CONFIG['adapters'][0][j]:
            j += 1
        else:
